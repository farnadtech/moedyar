    ...
    try {
      ...
      const config = {
        ...
        sms: {
          ...
          enabled: !!(
            envVars.SMS_USERNAME &&
            envVars.SMS_PASSWORD &&
      ...
      if (!section || section === "zarinpal") {
        if (config.zarinpal) {
          if (
            ...
            !config.zarinpal.merchantId.includes("••••")
          ) {
        ...
        case "zarinpal":
          try {
            const testPayment = await requestPayment({
              ...
              description: "تست اتصال درگاه پرداخت",
