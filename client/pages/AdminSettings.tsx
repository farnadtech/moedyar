...
import { 
  Settings, ArrowRight, Mail, MessageSquare, CreditCard, 
  Smartphone, Eye, EyeOff, Save, TestTube, CheckCircle, 
  AlertCircle, Zap, Users, FileText
} from "lucide-react";
...
export default function AdminSettings() {
    ...
    try {
      ...
      if (response.success) {
        toast({
          ...
          description: section ? `بخش ${section} با موفقیت به‌روزرسانی شد` : "تمام تنظیمات ذخیره شد"
        });
      } else {
        toast({
  ...
  const testService = async (service: 'email' | 'sms' | 'whatsapp' | 'zarinpal') => {
    try {
      setTesting(service);
      ...
      if (response.success) {
        ...
        toast({
          title: "❌ تست ناموفق",
          description: response.message || `خطا در تست سرویس ${getServiceName(service)}`,
          variant: "destructive"
        });
      }
    } catch (error) {
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xl font-bold text-gray-900">تنظیمات سیستم</span>
            </div>
          </div>
        </div>
      </header>
          ...
          <TabsContent value="email">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <Label htmlFor="email-password">رمز عبور یا App Password</Label>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateConfig('sms', 'username', e.target.value)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateConfig('sms', 'sender', e.target.value)}
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateConfig('sms', 'password', e.target.value)}
                  />
                </div>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    className="w-4 h-4"
                  />
                  <Label htmlFor="whatsapp-enabled">فعال‌سازی ارسال واتس‌اپ</Label>
                </div>
                ...
                <div className="flex gap-3">
                  <Button onClick={() => saveConfig('whatsapp')} disabled={saving}>
                    <Save className="w-4 h-4 mr-2" />
                    {saving ? "در حال ذخیره..." : "ذخیره"}
                  </Button>
                  <Button 
                    ...
                    disabled={testing === 'whatsapp' || !config.whatsapp.enabled}
                  >
                    <TestTube className="w-4 h-4 mr-2" />
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <Badge variant={config.zarinpal.enabled ? "default" : "secondary"}>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => updateConfig('zarinpal', 'enabled', e.target.checked)}
                    className="w-4 h-4"
                  />
                  <Label htmlFor="zarinpal-enabled">فعال‌سازی درگاه پرداخت</Label>
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateConfig('zarinpal', 'merchantId', e.target.value)}
                  />
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => updateConfig('zarinpal', 'sandbox', e.target.checked)}
                    className="w-4 h-4"
                  />
                ...
                <div className="flex gap-3">
                  <Button onClick={() => saveConfig('zarinpal')} disabled={saving}>
                    <Save className="w-4 h-4 mr-2" />
                    {saving ? "در حال ذخیره..." : "ذخیره"}
                  </Button>
                  <Button 
                    ...
                    disabled={testing === 'zarinpal' || !config.zarinpal.enabled}
                  >
                    <TestTube className="w-4 h-4 mr-2" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateConfig('app', 'supportEmail', e.target.value)}
                    />
                  </div>
                </div>
